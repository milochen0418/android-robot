...
public class Song implements Comparable<Song> {
	...
	public void populate(Cursor cursor)
	{
		...
		//id = cursor.getLong(0);
		id = this.getLongWithCatch(cursor, 0);
		
		//path = cursor.getString(1);
		path = this.getStringWithCatch(cursor,1);
		
		//title = cursor.getString(2);
		title = this.getStringWithCatch(cursor,2);
		
		//album = cursor.getString(3);
		album = this.getStringWithCatch(cursor, 3);
		
		//artist = cursor.getString(4);
		artist = this.getStringWithCatch(cursor,4);
		
		//albumId = cursor.getLong(5);
		albumId = this.getLongWithCatch(cursor, 5);
		
		//artistId = cursor.getLong(6);
		artistId = this.getLongWithCatch(cursor,6);
		
		//duration = cursor.getLong(7);
		duration = this.getLongWithCatch(cursor,7);
		
		//trackNumber = cursor.getInt(8);
		trackNumber = this.getIntWithCatch(cursor,8);
	}
	public String getStringWithCatch(Cursor cursor, int idx) {
		try {
	...
	public long getLongWithCatch(Cursor cursor, int idx) {
		try {
	...
	public int getIntWithCatch(Cursor cursor, int idx) {
		try {
